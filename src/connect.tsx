...
export async function main(ns: NS): Promise<void> {
  ...
  } = parseArgs<ConnectFlags, string[]>(ns, ConnectFlags);
  ...
  const printLimit = isVerbose ? undefined : 1;
  const slicedServers = sortedServers.slice(0, printLimit).reverse();
  for (const server of slicedServers) {
...
export function autocomplete(data: AutocompleteData, args: string[]): string[] {
  ...
  if (isCompletingFlag) {
    const flags = Object.keys(new ConnectFlags());
